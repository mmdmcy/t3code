...
describe("getAutoUpdateDisabledReason", () => {
  ...
  it("keeps packaged builds offline by default even with an update feed", () => {
    expect(
      getAutoUpdateDisabledReason({
        isDevelopment: false,
        isPackaged: true,
    ...
    ).toContain("T3CODE_ENABLE_AUTO_UPDATE");
  });

  it("allows packaged builds with an update feed when explicitly enabled", () => {
    expect(
      getAutoUpdateDisabledReason({
        isDevelopment: false,
        isPackaged: true,
