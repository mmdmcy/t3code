...
function decodeIconDataUrl(iconUrl: string): string {
  assert.isTrue(iconUrl.startsWith("data:image/svg+xml;charset=utf-8,"));
  return decodeURIComponent(iconUrl.split(",", 2)[1] ?? "");
}

function assertIconSource(iconUrl: string, expectedFilename: string): void {
  assert.include(decodeIconDataUrl(iconUrl), `data-source="${expectedFilename}"`);
}
...
describe("getVscodeIconUrlForEntry", () => {
  it("uses exact filename matches from the vscode-icons manifest", () => {
    ...
    assertIconSource(darkUrl, "file_type_pnpm.svg");
    assertIconSource(lightUrl, "file_type_light_pnpm.svg");
  });
  ...
  it("uses longest extension match for compound extensions", () => {
    ...
    assertIconSource(iconUrl, "file_type_tsbuildinfo.svg");
    assertIconSource(manifestStyleUrl, "file_type_buf.svg");
  });
  ...
  it("uses folder mappings and light-aware filename mappings", () => {
    ...
    assertIconSource(folderUrl, "folder_type_src.svg");
    assertIconSource(fileUrl, "file_type_light_agents.svg");
  });
  ...
  it("falls back to language-based mappings for path-only cases", () => {
    ...
    assertIconSource(tsxUrl, "file_type_reactts.svg");
    assertIconSource(dockerfileUrl, "file_type_docker.svg");
    assertIconSource(shellUrl, "file_type_shell.svg");
    assertIconSource(htmlUrl, "file_type_html.svg");
    assertIconSource(cursorRulesUrl, "file_type_markdown.svg");
    assertIconSource(githubWorkflowUrl, "file_type_light_yaml.svg");
  });
  ...
  it("falls back to defaults when there is no match", () => {
    ...
    assertIconSource(fileUrl, "default_file.svg");
    assertIconSource(folderUrl, "default_folder.svg");
  });
});
